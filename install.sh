...
echo "cd $CURRENT_DIR/bundle/YouCompleteMe/ && python3 install.py --clang-completer --go-completer"
cd $CURRENT_DIR/bundle/YouCompleteMe/
git submodule update --init --recursive
if [ `which clang` ]   # check system clang
then
    python3 install.py --clang-completer --go-completer --system-libclang   # use system clang
else
